        ...
        <div class="mb-5 flex-1">
            ...
            <div class="max-w-lg mx-auto flex mb-5">
                <select id="from" name="from_currency"
                    ...
                    <option value="MYR" {{ isset($value['from']) && $value['from'] == 'MYR' ? 'selected' : '' }}>
                        MYR - Malaysian Ringgit
                    </option>
                    <option value="PHP" {{ isset($value['from']) && $value['from'] == 'PHP' ? 'selected' : '' }}>
                        PHP - Phillipine Peso
                    </option>
                    <option value="SGD" {{ isset($value['from']) && $value['from'] == 'SGD' ? 'selected' : '' }}>
            ...
            <input name="amount" type='number' step="any" min="0.00" value="{{ $value['amount'] ?? 1 }}"
        ...
        <div class="mb-5 flex-1">
            ...
            <div class="max-w-lg mx-auto flex mb-5">
                <select id="to" name="to_currency"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-lg rounded-lg block flex-1 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white focus:ring-0 focus:border-0">
                    <option value="USD" {{ isset($value['to']) && $value['to'] == 'USD' ? 'selected' : '' }}>
                        USD - US Dollar
                    </option>
                    <option selected value="MYR"
                        {{ isset($value['to']) && $value['to'] == 'MYR' ? 'selected' : '' }}>
                        MYR - Malaysia Ringgit
                    </option>
                    <option value="PHP" {{ isset($value['to']) && $value['to'] == 'PHP' ? 'selected' : '' }}>
                        PHP - Phillipine Peso
                    </option>
                    <option value="SGD" {{ isset($value['to']) && $value['to'] == 'SGD' ? 'selected' : '' }}>
                        SGD - Singapore Dollar
                    </option>
            ...
            <input type="number" class="w-full border-0 outline-0 text-5xl font-number"
                value="{{ $value['result'] ?? '4.3905' }}" disabled>
        </div>
    </div>

<x-layout>
    <div class="min-h-[650px] mt-[32px] px-4">
        <div
            ...
            <form id="conversion-form" action="{{ route('convert') }}" method="POST">
                @csrf
                <div class="flex flex-col sm:flex-row items-center gap-5">
                    <div class="mb-5 flex-1">
                        <label for="from"
                            class="block mb-2 text-xl font-medium text-gray-500 dark:text-white">From</label>
                        <div class="max-w-lg mx-auto flex mb-5">
                            <select id="from" name="from_currency"
                                ...
                                <option value="EUR"
                                    {{ isset($value['from']) && $value['from'] == 'EUR' ? 'selected' : '' }}>
                                    EUR - Euro
                                </option>
                                <option value="USD"
                                    {{ isset($value['from']) && $value['from'] == 'USD' ? 'selected' : '' }}>
                                    USD - US Dollar
                                </option>
                                <option value="JPY"
                                    {{ isset($value['from']) && $value['from'] == 'JPY' ? 'selected' : '' }}>
                                    JPY - Japanese Yen
                                </option>
                                <option value="INR"
                                    {{ isset($value['from']) && $value['from'] == 'INR' ? 'selected' : '' }}>
                                    INR - Indian Rupee
                                </option>
                                <option value="THB"
                                    {{ isset($value['from']) && $value['from'] == 'THB' ? 'selected' : '' }}>
                                    THB - Thai Baht
                                </option>
                            </select>
                        </div>
                    ...
                    <div>
                        <button id="switch" type="button"
                            class="rounded-full p-3 hover:bg-slate-100 transition-colors">
                    ...
                    <div class="mb-5 flex-1">
                        <label for="to"
                            class="block mb-2 text-xl font-medium text-gray-500 dark:text-white">Converted
                            to</label>
                        <div class="max-w-lg mx-auto flex mb-5">
                            <select id="to" name="to_currency"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-lg rounded-lg block flex-1 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white focus:ring-0 focus:border-0">
                                <option value="EUR"
                                    {{ isset($value['to']) && $value['to'] == 'EUR' ? 'selected' : '' }}>
                                    EUR - Euro
                                </option>
                                ...
                                <option value="JPY"
                                    {{ isset($value['to']) && $value['to'] == 'JPY' ? 'selected' : '' }}>
                                    JPY - Japanese Yen
                                </option>
                                <option value="INR"
                                    {{ isset($value['to']) && $value['to'] == 'INR' ? 'selected' : '' }}>
                                    INR - Indian Rupee
                                </option>
                                <option value="THB"
                                    {{ isset($value['to']) && $value['to'] == 'THB' ? 'selected' : '' }}>
                                    THB - Thai Baht
                                </option>
